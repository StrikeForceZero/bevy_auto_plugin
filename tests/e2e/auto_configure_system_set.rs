...
use bevy_auto_plugin_proc_macros::auto_configure_system_set;
...
#[auto_configure_system_set(plugin = TestPlugin, schedule = Update, chain)]
enum TestSet {
    A,
    B,
    C,
...
fn test_auto_configure_system_set() {
    ...
    app.update();
    assert_eq!(get_run_log(&app), &["a", "b", "c"]);
}

...
pub struct RunOnBuildAttributeArgs {
    #[darling(multiple)]
    pub generics: Vec<TypeList>,
}

impl AutoPluginAttributeKind for RunOnBuildAttributeArgs {
...
impl ItemAttributeArgs for RunOnBuildAttributeArgs {
    fn global_build_prefix() -> &'static str {
        "_global_auto_plugin_run_on_build_"
    }
    ...
    fn match_items(items: &[Item]) -> syn::Result<Vec<ItemWithAttributeMatch<'_, Self>>> {
        items_with_attribute_match::<FnMeta, RunOnBuildAttributeArgs>(items)
    }
}

impl GenericsArgs for RunOnBuildAttributeArgs {
...
impl ToTokensWithConcreteTargetPath for RunOnBuildAttributeArgs {
...
impl ArgsBackToTokens for RunOnBuildAttributeArgs {
...
mod tests {
    ...
    fn test_to_tokens_no_generics() -> syn::Result<()> {
        let args = parse2::<RunOnBuildAttributeArgs>(quote!())?;
    ...
    fn test_to_tokens_single() -> syn::Result<()> {
        let args = parse2::<RunOnBuildAttributeArgs>(quote!(generics(u8, bool)))?;
        let path: Path = parse_quote!(foo_target);
        let args_with_target = WithTargetPath::try_from((path, args))?;
        let mut token_iter = args_with_target.to_tokens_iter();
        assert_eq!(
    ...
    fn test_to_tokens_multiple() -> syn::Result<()> {
        let args =
            parse2::<RunOnBuildAttributeArgs>(quote!(generics(u8, bool), generics(bool, bool)))?;
        let path: Path = parse_quote!(foo_target);
        let args_with_target = WithTargetPath::try_from((path, args))?;
        let mut token_iter = args_with_target.to_tokens_iter();
        assert_eq!(

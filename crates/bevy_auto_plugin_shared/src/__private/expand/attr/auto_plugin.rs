...
pub fn expand_auto_plugin(attr: MacroStream, input: MacroStream) -> MacroStream {
    ...
    let (auto_plugin_hook, auto_plugin_hook_end) = if let Some(self_arg) = self_arg {
        ...
        (
            ...
            quote! {
                <Self as ::bevy_auto_plugin::__private::shared::__private::auto_plugin_registry::AutoPlugin>::build_end(#self_arg, #app_param_ident);
            },
        )
    } else {
        if sig.inputs.len() > 1 {
        ...
        (
            ...
            quote! {
                <#plugin_ident as ::bevy_auto_plugin::__private::shared::__private::auto_plugin_registry::AutoPlugin>::static_build_end(#app_param_ident);
            },
        )
    };

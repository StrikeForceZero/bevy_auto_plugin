...
pub fn args_with_plugin_from_args_input(
    ...
    #[derive(Default)]
    struct PerVariant {
        default: Option<ConfigureSystemSetArgsInnerEntry>, // group == None
        per_group: HashMap<Ident, ConfigureSystemSetArgsInnerEntry>, // group == Some(g)
    }

    // We will fill this keyed by variant ident
    let mut variants_cfg: HashMap<Ident, PerVariant> = HashMap::new();

    // We need an observed order per variant, based on site enumeration position.
    // We'll track the FIRST observed index we see for that variant.
    ...
    // Your requirement says: "maybe i throw an error if i see 2 of the same top level groups".
    // We'll treat a second helper on the same (variant, group or None) as a hard error.
